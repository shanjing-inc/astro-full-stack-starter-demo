...
import {
    configureSuperAdminGraphQL,
    executeSuperAdminGraphQL,
    getSuperAdminGraphQLUri,
...
describe("executeSuperAdminGraphQL", () => {
    beforeEach(() => {
        vi.useFakeTimers();
        configureSuperAdminGraphQL({
            uri: null,
    ...
    it("batches concurrent super admin operations into one request", async () => {
        ...
        const firstRequest = executeSuperAdminGraphQL<{
            getFirstBatchItem: {
                id: string;
            };
        }>(FIRST_QUERY);
        const secondRequest = executeSuperAdminGraphQL<{
            getSecondBatchItem: {
                id: string;
            };
        }>(SECOND_QUERY);
    ...
    it("uses configured super admin GraphQL endpoint paths", async () => {
        ...
        const request = executeSuperAdminGraphQL<{
            getFirstBatchItem: {
                id: string;
            };
        }>(FIRST_QUERY);
        ...
        expect(getSuperAdminGraphQLUri()).toBe("/api/graphql/custom-admin");
    });
    ...
    it("rejects GraphQL errors returned by the server", async () => {
        ...
        const request = executeSuperAdminGraphQL(FIRST_QUERY);
    ...
    it("rejects mocked links that complete without data", async () => {
        ...
        const { executeSuperAdminGraphQL: executeWithMockedLink } =
            await import("@shanjing/astro-full-stack-starter/super-admin/client");
