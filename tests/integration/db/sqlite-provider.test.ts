...
import { createBetterSqliteDb } from "@shanjing/astro-full-stack-starter/db/sqlite/client";
...
describe("Deno MySQL demo SQLite test provider", () => {
    it("uses a fixture schema and relations for SQLite tests", () => {
        ...
        expect(provider.schema).toBe(sqliteDbSchema);
        expect(provider.relations).toBe(sqliteDbRelations);
    ...
    it("uses sqlite table metadata for fixture business tables", () => {
        const provider = createSqliteTestDatabaseProvider();
    ...
    it("creates a better-sqlite3 backed file database", () => {
        const tempDir = mkdtempSync(join(tmpdir(), "sqlite-provider-"));
        const databasePath = join(tempDir, "demo.sqlite");

        try {
            const db = createBetterSqliteDb({
                relations: sqliteDbRelations,
                schema: sqliteDbSchema,
                source: databasePath,
            });
            db.$client.exec("create table smoke (id integer primary key, value text not null);");
            db.$client.prepare("insert into smoke (value) values (?)").run("persisted");
            db.$client.close();

            const reopenedDb = createBetterSqliteDb({
                relations: sqliteDbRelations,
                schema: sqliteDbSchema,
                source: databasePath,
            });
            ...
            expect(
                reopenedDb.$client.prepare("select value from smoke where id = 1").get()
            ).toEqual({
                value: "persisted",
            });
    ...
    it("initializes tables from generated schema SQL and lets tests insert records", () => {
        const provider = createSqliteTestDatabaseProvider();
        const db = provider.getDb();
        const records = createShopOrderFixture();
