...
const createShopSetSchema = z.object({
    name: z.string().min(1),
    slug: z.string().min(1),
    status: z.enum(shopStatusEnum).optional(),
});
const updateShopSetSchema = z
    .object({
...
export function buildRequiredShopWhereClause(
    ...
    if (!whereClause) {
        throw new GraphQLError("Shop filters require at least one where condition.");
    }
