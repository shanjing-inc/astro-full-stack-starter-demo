...
const createOrderSetSchema = createInsertSchema(order).pick({
    shopId: true,
    productId: true,
    orderNo: true,
    quantity: true,
...
const updateOrderSetSchema = createUpdateSchema(order)
    .pick({
        shopId: true,
        productId: true,
        orderNo: true,
...
export function buildRequiredOrderWhereClause(
    ...
    if (!whereClause) {
        throw new Error("Order filters require at least one where condition.");
    }
